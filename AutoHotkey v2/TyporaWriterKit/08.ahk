...
ShowMainGui()
{
    ...
    ; 这里的 winH 暂时只作为初始值，后续会根据内容自动调整或固定
    ...
    ; === 创建窗口 (禁止改变大小) ===
    MainGui := Gui("+MinimizeBox -MaximizeBox -Resize", "Typora 终极助手")
    ...
    ; 计算 GroupBox 的高度
    ; 先预估高度，或者直接画
    ; 更好的方式：不画 GroupBox，而是用清晰的对齐
    ; 或者画一个 GroupBox 包裹所有输入项
    
    currentY := topMargin + 25 ; 从 GroupBox 内部开始
    
    ; 先添加一个 GroupBox 占位 (高度稍后调整，或者直接足够大)
    ; 这里我们为了布局整洁，让 GroupBox 作为一个视觉容器
    gbStart := MainGui.Add("GroupBox", "x" sideMargin " y" topMargin " w" (winW - sideMargin*2) " h500", " 文章属性 ")
    
    gbInnerY := topMargin + 30 ; 内容起始Y
    ...
    for index, fieldName in fields 
    {
        ...
        if (fieldName = "Tags" || fieldName = "Categories") 
        {
            ...
            gbInnerY += 125 ; 增加 Y (80列表 + 26按钮 + 间距)
        } 
        else 
        {
            ; --- 单行模式 ---
...
{
    global SettingsGui
    InputGui := Gui("+Owner" SettingsGui.Hwnd " +AlwaysOnTop -MaximizeBox -Resize", title)
...
ShowColorHelpGui()
{
    ...
    ColorHelpGui.AddEdit(
        ...
        "③ 若文字消失，请按 Ctrl+Z 撤销。`n`n"
        "④ 如果想换色，请先删除旧颜色代码或撤销，再重新添加。"
    )
...
ShowColorTool()
{
    ...
    infoBtn.OnEvent("Click", (*) => ShowColorHelpGui()) ; 链接到专用说明书
...
ShowCustomColorGui()
{
    ...
    apply := CT_CustomGui.AddButton("xm y+6 w260", "使用该颜色")
