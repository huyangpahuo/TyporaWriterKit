...
ShowMainGui()
{
    ...
    winH := IniRead(IniFile, "Appearance", "WinHeight", 650)
    ...
    for index, fieldName in fields 
    {
        ...
        if (fieldName = "Tags" || fieldName = "Categories") 
        {
            ...
            lb := MainGui.Add("ListBox", "x+10 yp w" ctlW " h80", defArr)
            ListFieldControls[fieldName] := lb
            
            inputW := ctlW - 70
            addInput := MainGui.Add("Edit", "xp y+85 w" inputW, "")
...
ShowMainHelpGui()
{
    ...
    MainHelpGui.AddEdit(
        ...
        "【YAML 生成器使用指南】`n`n"
        "1. 在下方输入框填写文章信息。`n"
        "2. Tags 和 Categories 支持添   多个项目，点击 + 号添加。`n"
